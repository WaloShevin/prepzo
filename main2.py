...
os.environ['GOOGLE_API_KEY'] = 'AIzaSyBInTzakl-SGObIT5QZiR1J5ml8nD_TZSQ'
                    ...
                    while True:
                        try:
                            ...
                            async for response in session.receive():
                                ...
                                # Check if there is model turn content to process
                                model_turn = response.server_content.model_turn
                                if model_turn:
                                    for part in model_turn.parts:
                                        if hasattr(part, 'text') and part.text is not None:
                                            # Text data from Gemini
                                            print(f"Text received: {part.text}")
                                            await client_websocket.send(json.dumps({"text": part.text}))

                                        elif hasattr(part, 'inline_data') and part.inline_data is not None:
                                            # Check if the part has audio content
                                            if part.inline_data.mime_type.startswith("audio"):
                                                ...
                                                print("Audio data received and forwarded")
                                            else:
                                                print("Received inline data, but it's not audio")
